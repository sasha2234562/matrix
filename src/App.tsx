...
function App() {
  ...
  useEffect(() => {
    ...
    if (existingTerms) {
      const existingTermsParse: Term = JSON.parse(existingTerms);
      setSelectWatchCurrencies(existingTermsParse);
    }
  }, []);

  useEffect(() => {
    if (Object.keys(selectWatchCurrencies).length === 0) {
      return;
    }
  ...
  const onClickRemoveCurrency = useCallback((symbol: string) => {
    if (existingTerms) {
      const existingTermsParsed: Term = JSON.parse(existingTerms);
      const tickersAll = tickers;
      delete tickers[symbol];
      delete existingTermsParsed[symbol];
      localStorage.setItem('terms', JSON.stringify(existingTermsParsed));
      setSelectWatchCurrencies(existingTermsParsed);
      setTickers(tickersAll);
    }
  }, [existingTerms, tickers]);
          ...
          {Object.values(tickers).map(item => {
            ...
            return (
              <SelectedTrackedCurrency
                ...
                onClickRemoveCurrency={onClickRemoveCurrency}
              />
            );
          })}
        </div>
